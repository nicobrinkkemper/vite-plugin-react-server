...
describe('Error Boundaries (Cross-Environment)', () => {
  ...
  beforeAll(async () => {
  const buildResult = await getSharedBuild('error-boundaries', {
    setupProject: setupErrorBoundaryTestProject,
    pages: ['/'], // Only build the main page to avoid error pages hanging
  });
  ...
  it('should generate error boundary components in static output', () => {
    ...
    const errorBoundaryFiles = staticFiles.filter(f => 
  ...
  it('should include error handling in server build output', () => {
    ...
    const errorHandlingFiles = serverFiles.filter(f => 
