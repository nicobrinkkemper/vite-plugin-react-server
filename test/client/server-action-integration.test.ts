...
const testDir = resolve(__dirname, '../fixtures/client-server-action-integration.test');
...
describe('Client Server Action Integration', () => {
  ...
  it('should include server action references in the RSC stream', async () => {
    expect(response.result).toContain('"id":"/src/page/actions.server.ts#addTodo"');
    expect(response.result).toContain('"id":"/src/page/actions.server.ts#toggleTodo"');
    expect(response.result).toContain('"id":"/src/page/actions.server.ts#deleteTodo"');
    expect(response.result).toContain('"id":"/src/page/actions.server.ts#getTodos"');
  });
  ...
  it('should handle client component references correctly', async () => {
    ...
    expect(response.result).toContain('7:I["src/components/TodoList.client.tsx","TodoList"]');
  });
}); 