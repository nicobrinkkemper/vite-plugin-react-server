...
export async function transformServerModule(
  ...
  loader: ResolvedUserOptions['loader'] = DEFAULT_LOADER_CONFIG,
  verbose = false
  ...
  function walkFunctionDirectives(node: any) {
    if (
      ...
      node.body && node.body.type === 'BlockStatement' && node.body.body.length > 0
    ) {
  ...
  for (const exp of parseResult.exports.exports.values()) {
    ...
    if (hasServerDirective || parseResult.directiveInfo.fileLevel?.type === "server") {
      registrations.push(
  ...
  const finalCode = registrations.length > 0
