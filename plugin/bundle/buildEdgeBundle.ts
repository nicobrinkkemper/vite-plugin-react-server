...
`;
  ...
  try {
    ...
    // Types for the bundle, emitted AFTER the build (it runs with emptyOutDir,
    // so anything written earlier is wiped). The bundle is generated JS with no
    // declarations of its own, which makes the one-line consumer —
    // `import * as bundle from "./dist/server-edge/render.js"` — a TS7016 error
    // under `strict`: "could not find a declaration file". Generating the module
    // means generating its types too; otherwise every TypeScript consumer
    // hand-writes a .d.ts shim, which is the boilerplate this is meant to end.
    //
    // Shapes are derived from vprs's own EdgeBundleExports rather than restated,
    // so the bundle stays assignable to what createEdgeRequestHandler accepts and
    // the two cannot drift.
    writeFileSync(
      join(edgeDir, entryFileName.replace(/\.js$/, ".d.ts")),
      `// Generated by vite-plugin-react-server (build.edge) — do not edit.
import type { EdgeBundleExports } from "vite-plugin-react-server/edge";

/** Headless page flight (Page only). */
export declare const ${flightExport}: (
  url: string,
  request?: Request
) => Promise<ReadableStream<Uint8Array>>;

/** Full-document flights: the Html-wrapped \`full\` + the Root-only \`headless\`. */
export declare const ${documentExport}: EdgeBundleExports[${JSON.stringify(
        "renderRouteToDocument"
      )}];

/** Baked server-action gate: dispatches without a \`react-server\` condition. */
export declare const ${actionExport}: NonNullable<
  EdgeBundleExports[${JSON.stringify("handleRouteAction")}]
>;

/** The content-hashed browser entry, baked from the client manifest. */
export declare const ${bootstrapExport}: string[];

/** The built client bundle, resolved relative to this module. */
export declare const ${clientBaseExport}: string;
`,
      "utf8"
    );
