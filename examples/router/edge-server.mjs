...
// the root, so the browser bootstrap url is `/<file>`.
...
const handler = createEdgeHandler({
  ...
  // The IN-PROCESS render resolves client-component references by importing
  // them, so this must be a FILE url to the built client chunks (dist/client),
  // NOT the browser's http base. (The browser hydrates from `/` separately, via
  // bootstrapModules.) `"/"` only works for a page with no client components.
  moduleBaseURL: pathToFileURL(clientDir).href + "/",
  bootstrapModules,
});
...
createServer(async (req, res) => {
  ...
  console.log("edge demo running on http://localhost:8788");
});
