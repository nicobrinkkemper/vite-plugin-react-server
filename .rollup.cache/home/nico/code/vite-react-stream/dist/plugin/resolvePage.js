export async function resolvePage({ pageModule, path, url, exportName, }) {
    if (!pageModule) {
        return {
            type: "error",
    ...
    const keys = typeof pageModule === "object" && pageModule != null
    ...
    return {
        type: "error",
        error: new Error(`Could not find Page export "${exportName}" in "${path}". ${typeof pageModule === "object" && pageModule != null
            ? keys.length
                ? "Available exports: " + keys.join(", ")
                : "The object was defined but has no properties."
            : "typeof pageModule =" + typeof pageModule}`, { cause: pageModule }),
    };
}
