...
This guide covers the worker system architecture, including RSC workers, HTML workers, and worker communication patterns.

## Worker Architecture

The plugin uses a worker-based architecture to handle React Server Components processing and HTML generation. This provides isolation, performance benefits, and the ability to run different React conditions in separate environments.
