...
The plugin provides two development modes with **identical user experiences** but different internal architectures:

```mermaid
graph TB
    subgraph "RSC Worker Mode"
        A1[Command: vite<br/>Condition: null<br/>Main Thread: Standard Node.js]
        A2[RSC Worker Thread<br/>Condition: react-server]
        A3[Browser: Same User Experience]
        A1 --> A2
        A2 --> A3
    end
```
...
```mermaid
graph TB
   subgraph "Direct Server Mode"
       B1[Command: NODE_OPTIONS=--conditions react-server vite<br/>Condition: react-server<br/>Main Thread: RSC Environment]
       B2[Direct RSC Processing<br/>No Worker Overhead]
       B3[Browser: Same User Experience]
       B1 --> B2
       B2 --> B3
   end
```
...
graph TB
 subgraph "Build Environment"
     C1[Command: npm run build<br/>Condition: react-server<br/>Main Thread: Build-time]
     C2[Static Build<br/>vite build]
     C3[Client Build<br/>vite build --ssr]
     C4[Server Build<br/>NODE_OPTIONS=--conditions react-server vite build --ssr]
     C5[HTML Worker<br/>Only during builds]
     C6[dist/static/<br/>index.html + index.rsc]
     C1 --> C2
     C1 --> C3
     C1 --> C4
     C4 --> C5
...
- Both development modes provide identical browser experiences
- Client mode uses worker thread for RSC processing
- Server mode processes RSC directly in main thread (no worker overhead)
- HTML worker is only used during builds, not development
- No RSC worker → HTML worker communication in development
...
- **Full Stream (config's Html Component)**:
...
Both streams provide detailed stack traces when run under `NODE_ENV=development`. In the case of the
`html-worker` this also includes the component stack. Otherwise you can expect the component stack
in your browser. Since the build runs under production by default, you will have to set NODE_ENV explicitly. The mode will automatically be set to development because the NODE_ENV takes precedence, but to avoid a warning about this you should also explicitly set the vite mode to development.

5. **Preview Server MIME Type Handling**:

- The preview server (used after builds) handles MIME types for static files
- RSC (React Server Component) files use `text/x-component` MIME type
- HTML files are served with `text/html` MIME type
- The `configurePreviewServer` middleware routes requests based on file extensions
- Only handles RSC requests (`isRscRequest`) for static file serving
